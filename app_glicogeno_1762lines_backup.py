...
def main():
    ...
    with tab3:
        ...
        with c_param:
            ...
            # NUOVA FUNZIONALITÀ: OVERRIDE TANK (TEST MODE)
            st.subheader("0. Override Manuale (Test Mode)")
            override_tank = st.checkbox("Forza Riempimento Iniziale (Test)")
            
            if override_tank:
                # Recupera il valore totale attuale come default
                current_total_g = tank['actual_available_g']
                forced_total_g = st.number_input("Glicogeno Totale (g)", 100, 1000, int(current_total_g), 10)
                
                # Ricalcola muscolo e fegato proporzionalmente o con logica fissa
                # Assumiamo un fegato "sano" a ~100g e il resto muscolo, a meno che il totale sia basso
                forced_liver_g = min(100.0, forced_total_g * 0.2) # Max 100g o 20% del totale
                forced_muscle_g = forced_total_g - forced_liver_g
                
                # Sovrascrive temporaneamente i valori per la simulazione
                tank_simulation_g = forced_total_g
                tank_muscle_g = forced_muscle_g
                tank_liver_g = forced_liver_g
                st.warning(f"Simulazione con valori forzati: Muscolo {int(tank_muscle_g)}g, Fegato {int(tank_liver_g)}g")
            else:
                # Usa i valori calcolati dal Tab 2
                tank_simulation_g = tank['actual_available_g']
                tank_muscle_g = tank['muscle_glycogen_g']
                tank_liver_g = tank['liver_glycogen_g']
        ...
        df_sim, stats = simulate_metabolism_cached(
            tank_g=tank_simulation_g, # USA IL TANK (FORZATO O CALCOLATO)
            initial_muscle_g=tank_muscle_g, # USA IL TANK (FORZATO O CALCOLATO)
            initial_liver_g=tank_liver_g,   # USA IL TANK (FORZATO O CALCOLATO)
