...
def ml_optimize_parameters(training_data, ftp):
    """
    Trova i migliori parametri (Crossover, Efficiency) che minimizzano l'errore
    rispetto ai dati reali forniti (Watt vs CHO rate).
    Usa un approccio Grid Search (Brute Force intelligente) robusto.
    """
    best_error = float('inf')
    best_params = {'crossover': 75, 'efficiency': 22.0}
    
    # Grid di ricerca
    crossover_range = range(50, 86, 1) # 50% a 85%
    efficiency_range = np.arange(16.0, 26.5, 0.5) # 16% a 26%
    
    for co in crossover_range:
        for eff in efficiency_range:
            ...
            for point in training_data:
                ...
                # Predizione Modello (Versione light di simulate_metabolism per t=0)
                intensity_factor = watts / ftp if ftp > 0 else 0
                ...
                kcal_min = (watts * 60) / 4184 / (eff / 100.0)
                pred_cho = (kcal_min * cho_ratio) / 4.1
                
                total_sq_error += (pred_cho - real_cho) ** 2
            
            rmse = math.sqrt(total_sq_error / len(training_data))
            
            if rmse < best_error:
                best_error = rmse
                best_params = {'crossover': co, 'efficiency': eff}
...
tab1, tab2, tab3 = st.tabs(["Analisi Riserve (Tank)", "Simulazione Metabolica (Burn)", "🤖 ML Tuning (Beta)"])
...
with tab1:
    ...
    with col_in:
        ...
        for d in DietType:
            ...
            label = f"{d.label} (> {d.ref_value} g/kg)"
    ...
    if 'tank_g' not in st.session_state:
        ...
        with col_param:
            ...
            if sport_mode == 'cycling':
                ...
                # Override efficiency se calibrata
                default_eff = st.session_state.get('learned_efficiency', 22.0)
                act_params['efficiency'] = st.slider("Efficienza Meccanica [%]", 16.0, 26.0, default_eff, 0.5)
                duration = st.slider("Durata Attività (min)", 30, 420, 120, step=10)
                
            elif sport_mode == 'running':
                # ... (Codice running invariato)
                act_params['efficiency'] = 22.0 # Dummy per running
        ...
        with col_meta:
            ...
            if use_lab:
                ...
                # Override crossover se calibrato
                default_co = st.session_state.get('learned_crossover', 75)
        ...
        # ... (Visualizzazioni Grafiche esistenti) ...
        ...
        with g1:
            ...
            chart_stack = alt.Chart(df_long).mark_area().encode(
                x='Time (min)', y='Rate (g/h)', color='Source', tooltip=['Time (min)', 'Source', 'Rate (g/h)']
            ).interactive()
            st.altair_chart(chart_stack, use_container_width=True)

        with g2:
            st.caption("Accumulo Intestinale")
            gut_chart = alt.Chart(df_sim).mark_area(color='#8D6E63', opacity=0.6).encode(x='Time (min)', y='Gut Load')
...
# --- TAB 3: ML TUNING ---
with tab3:
    st.header("🤖 Digital Twin Tuning (Machine Learning)")
    st.markdown("""
    Usa questa sezione per calibrare i parametri nascosti del modello (Crossover Point ed Efficienza) 
    basandoti su dati reali raccolti sul campo o in laboratorio.
    """)
    
    st.subheader("1. Inserimento Dati Reali")
    st.caption("Inserisci 2 o 3 punti noti (Potenza vs Consumo CHO) da un test metabolico o storico gare.")
    
    if 'training_data' not in st.session_state:
        st.session_state['training_data'] = []
        
    c1, c2 = st.columns(2)
    in_watts = c1.number_input("Potenza (Watt)", 100, 600, 200, step=10)
    in_cho = c2.number_input("Consumo CHO Reale (g/min)", 0.0, 5.0, 2.0, 0.1)
    
    if st.button("Aggiungi Punto Dati"):
        st.session_state['training_data'].append({'watts': in_watts, 'cho': in_cho})
        
    # Tabella Dati
    if st.session_state['training_data']:
        st.table(pd.DataFrame(st.session_state['training_data']))
        
        if st.button("🚀 Esegui Tuning Automatico"):
            if sport_mode != 'cycling':
                st.warning("Il tuning automatico funziona al meglio con dati di potenza (Ciclismo).")
            else:
                # Esegui ottimizzazione
                # Recuperiamo FTP attuale per calcolare Intensity Factor
                current_ftp = act_params.get('ftp_watts', 250)
                
                with st.spinner("Ottimizzazione parametri in corso..."):
                    best_params, error = ml_optimize_parameters(st.session_state['training_data'], current_ftp)
                
                st.success(f"Tuning Completato! Errore Medio (RMSE): {error:.3f} g/min")
                st.write(f"### Parametri Ottimizzati Trovati:")
                c_res1, c_res2 = st.columns(2)
                c_res1.metric("Crossover Point Ideale", f"{best_params['crossover']}%")
                c_res2.metric("Efficienza Meccanica Reale", f"{best_params['efficiency']:.1f}%")
                
                # Applicazione
                if st.button("Applica al Modello"):
                    st.session_state['learned_crossover'] = best_params['crossover']
                    st.session_state['learned_efficiency'] = best_params['efficiency']
                    st.rerun()
    else:
        st.info("Aggiungi almeno un punto dati per iniziare il tuning.")
        
    st.markdown("---")
    st.caption("Nota: L'algoritmo usa una Grid Search per minimizzare l'errore quadratico medio (RMSE) tra le predizioni del modello e i tuoi dati reali, trovando la combinazione unica di fisiologia che ti rappresenta.")
