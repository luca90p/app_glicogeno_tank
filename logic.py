...
# --- 1. FUNZIONI HELPER (IDENTICHE AL BACKUP) ---
...
def interpolate_consumption(current_val, curve_data):
    # Funzione mantenuta per compatibilità interfaccia, ma non usata nel calcolo core
    return 0, 0

# --- 2. MOTORE SIMULAZIONE (LOGICA BACKUP 1:1) ---
...
def simulate_metabolism(subject_data, duration_min, constant_carb_intake_g_h, cho_per_unit_g, crossover_pct, 
    ...
    for t in range(int(duration_min) + 1):
        ...
        # --- LOGICA DI CONSUMO (TEORICA RER) ---
        # Calcolo sempre col modello teorico RER come richiesto
        standard_crossover = 70.0 
        # Usa il crossover passato come argomento se disponibile, altrimenti default
        ...
        # --- RIPARTIZIONE (LOGICA COGGAN ORIGINALE) ---
        # La chiave è il muscle_contribution_factor basato sul riempimento
        ...
        # IMPORTANTISSIMO: Questa riga sotto impediva il gradino di crash totale, 
        # ma permette al fegato di svuotarsi. Se si svuota, from_liver diventa 0.
...
def calculate_tapering_trajectory(subject, days_data, start_state: GlycogenState = GlycogenState.NORMAL):
    ...
    for day in days_data:
        ...
        consumption = 0
        if duration > 0:
            consumption = (duration/60) * 600 * intensity 
        
        net = (cho_in * sleep_factor) - (LIVER_DRAIN_RATE * 24 + DAILY_NEAT_CHO + (consumption/4))
