...
def calculate_tapering_trajectory(subject, days_data, start_state: GlycogenState = GlycogenState.NORMAL):
    ...
    MAX_LIVER = 100.0 
    MAX_MUSCLE = tank['max_capacity_g'] - MAX_LIVER
...
# --- 3. MOTORE SIMULAZIONE GARA (TAB 3) ---
...
def simulate_metabolism(subject_data, duration_min, constant_carb_intake_g_h, cho_per_unit_g, crossover_pct, 
    ...
    for t in range(int(duration_min) + 1):
        ...
        # B. Consumo Energetico (Lab vs Teorico)
        if is_lab_data_active:
            ...
            # Motore Teorico (Backup Logic Restored)
            if mode == 'cycling':
            ...
            # STANDARD CROSSOVER RIPRISTINATO A 75.0 (Come nel Backup)
        ...
        # D. Ripartizione Fonti (Backup Logic + Failsafe)
        
        # Fading Muscolare (Coggan)
        ...
        # Nel backup, muscle_usage era la quota "desiderata"
        target_muscle_draw = total_cho_demand_g_min * fading_factor
        from_muscle = min(target_muscle_draw, current_muscle_glycogen)
        
        # Il resto è Blood Demand
        ...
        # 3. Failsafe (Solo per coprire il deficit se fegato/esogeno non bastano)
        # Questo impedisce il gradino ma rispetta le proporzioni del backup
        unmet_blood = remaining_blood_needed - from_liver
        if unmet_blood > 0 and current_muscle_glycogen > from_muscle:
             # Il muscolo copre l'extra solo se ha riserve
             extra_muscle = min(unmet_blood, current_muscle_glycogen - from_muscle)
