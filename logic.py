...
def calculate_filling_factor_from_diet(weight_kg, cho_d1, cho_d2, s_fatigue, s_sleep, steps_m1, min_act_m1, steps_m2, min_act_m2):
    ...
    # Peso maggiore al giorno -1 (70%) rispetto al giorno -2 (30%)
...
def calculate_tank(subject: Subject):
    ...
    Utilizza la massa muscolare attiva specifica per lo sport.
    """
    ...
    # Bonus creatina (+10% volume cellulare stimato)
    ...
    # Limite fisiologico massimo (saturazione)
...
def calculate_rer_polynomial(intensity_factor):
    """Polinomio di sesto grado per stimare il RER in funzione dell'Intensità."""
...
def simulate_metabolism(subject_data, duration_min, constant_carb_intake_g_h, cho_per_unit_g, crossover_pct, 
    ...
    Modella deplezione glicogeno muscolare/epatico, ossidazione esogena e carico intestinale.
    """
    results = []
    ...
    # Calcolo base domanda energetica (kcal/min)
    if mode == 'cycling':
    ...
    # Stima limite ossidazione esogena
    base_rate = 0.8 
    ...
    # Inizializzazione variabili accumulo
    total_fat_burned = 0.0
    gut_accumulation = 0.0
    current_exo_ox_rate = 0.0 
    alpha = 1 - np.exp(-1.0 / tau_absorption) # Costante di assorbimento
    ...
    # Gestione intake discreto (assunzione periodica)
    ...
    for t in range(int(duration_min) + 1):
        ...
        # Calcolo kcal demand istantaneo
        if mode == 'cycling':
            instant_power = current_if * ftp_watts
            # Drift di efficienza dopo 60 min
        ...
        if is_lab_data:
            ...
            # Shift metabolico (riduzione efficienza glicolitica nel tempo)
        ...
        # Ripartizione CHO (Muscolo vs Sangue/Fegato vs Esogeno)
        ...
        from_liver = min(blood_demand - from_exogenous, 1.2) # Max output epatico ~1.2 g/min
...
def calculate_weekly_balance(initial_muscle, initial_liver, max_muscle, max_liver, weekly_schedule, subject_weight, vo2max):
    """Calcola il bilancio settimanale del glicogeno."""
